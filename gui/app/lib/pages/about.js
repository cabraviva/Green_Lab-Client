  ...
  return `
    <div class="content">
      ...
      <ul>
        ...
        <li><a onclick="event.preventDefault();opn(this.href)" href="https://namemc.com/profile/MarderMitFlex">MarderMitFlex</a> ${isGerman() ? 'und' : 'and'} <a onclick="event.preventDefault();opn(this.href)" href="https://twitch.tv/scuffipuffi">Scuffipuffi</a> ${isGerman() ? 'für das testen des Clients' : 'for testing the client'}</li>
        <li>${isGerman() ? 'Und dich,' : 'And you,'} <a onclick="event.preventDefault();opn(this.href)" href="https://de.namemc.com/profile/${await window.getIGN()}">${await window.getIGN()}</a>${isGerman() ? ', weil du diesen Client benutzt' : ', for using this client!'}</li>
      </ul>
